...
async function run() {
  ...
  // Get Korea time for today's date
  const now = new Date();
  const todayStr = new Date(now.getTime() + (9 * 60 * 60 * 1000)).toISOString().slice(0, 10);
  const utcTodayStr = now.toISOString().slice(0, 10);
  
  // Actually, we want to look at ALL bonuses created in the last 12 hours, regardless of type
  const queryBody = {
    structuredQuery: {
  ...
  // Date string check (since the script was run today, could be UTC today or yesterday depending on time)
  // Let's just check the last 24 hours
  const oneDayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
  let bonuses = data.filter(d => d.document).map(d => fsDocToObj(d.document)).filter(b => b.createdAt >= oneDayAgo);
  
  const roiBonuses = bonuses.filter(b => b.type === 'roi' || b.type === 'roi_income');
  console.log(`Found ${roiBonuses.length} ROI bonuses in the last 24 hours.`);
  ...
  const userMap = {};
  for (const b of roiBonuses) {
    if (!userMap[b.userId]) userMap[b.userId] = { totalRoiUsdt: 0, count: 0, maxAmount: 0 };
    userMap[b.userId].totalRoiUsdt += (b.amountUsdt || 0);
    userMap[b.userId].count++;
    if ((b.amountUsdt || 0) > userMap[b.userId].maxAmount) {
      userMap[b.userId].maxAmount = b.amountUsdt || 0;
    }
  }
  ...
  const results = Object.keys(userMap).map(uid => {
    ...
    return {
      ...
      count: d.count,
      maxPayout: d.maxAmount.toFixed(2)
    };
  ...
  for (const r of results) {
    ...
    if (parseFloat(r.ratio) > 2) overPaid++;
  }
  
  console.log(`최근 24시간 총 지급된 ROI: $${totalPaid.toFixed(2)}`);
  console.log(`원금 대비 2% 이상 지급된 유저 수: ${overPaid}명`);
}
