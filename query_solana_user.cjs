...
async function run() {
  ...
  const users = await fsQueryFast('users', token, 1000);
  const match1 = users.find(u => u.solanaWallet === '2qo8jvuc49pFmTjmUHLiARSV6ppPTaE7gw27ZJ6DnNZy' || u.depositWalletAddress === '2qo8jvuc49pFmTjmUHLiARSV6ppPTaE7gw27ZJ6DnNZy');
  console.log("Matched User by address:", match1 ? match1.email : "Not found");
  
  const txs = await fsQueryFast('transactions', token, 5000);
  const pendingTxs = txs.filter(t => t.type === 'deposit' && (t.status === 'pending' || t.status === 'approved') && (t.txid === '3XkhEGEhgaYdg5vvkTFhaoLxeys616DL33UVZHfm25pe5pDkr12ynTFct4g7pHVBDRtYnrJ2wqZqDrDCFoszTTm9' || t.txHash === '3XkhEGEhgaYdg5vvkTFhaoLxeys616DL33UVZHfm25pe5pDkr12ynTFct4g7pHVBDRtYnrJ2wqZqDrDCFoszTTm9'));
  console.log("Found matching TXs in firestore:", pendingTxs.length, pendingTxs);
}
run();
