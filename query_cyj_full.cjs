const admin = require('firebase-admin');

const SERVICE_ACCOUNT = {
  project_id: "dedra-mlm",
  private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  client_email: "firebase-adminsdk-fbsvc@dedra-mlm.iam.gserviceaccount.com",
  token_uri: "https://oauth2.googleapis.com/token"
};
const jwt = require('jsonwebtoken');

async function getAdminToken() {
  const payload = {
    iss: SERVICE_ACCOUNT.client_email,
    sub: SERVICE_ACCOUNT.client_email,
    aud: SERVICE_ACCOUNT.token_uri,
    iat: Math.floor(Date.now() / 1000),
    exp: Math.floor(Date.now() / 1000) + 3600,
    scope: 'https://www.googleapis.com/auth/datastore'
  };
  const token = jwt.sign(payload, SERVICE_ACCOUNT.private_key, { algorithm: 'RS256' });
  const res = await fetch(SERVICE_ACCOUNT.token_uri, {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    body: `grant_type=urn:ietf:params:oauth:grant-type:jwt-bearer&assertion=${token}`
  });
  return (await res.json()).access_token;
}

function fromFirestoreValue(val) {
  if (!val) return null;
  if ('stringValue' in val) return val.stringValue;
  if ('doubleValue' in val) return val.doubleValue;
  if ('integerValue' in val) return parseInt(val.integerValue,10);
  if ('timestampValue' in val) return new Date(val.timestampValue);
  return val;
}

async function run() {
  const token = await getAdminToken();
  const uid = 'qAdGKU772oVGZ0B5PwUEbL3UqSF3'; // cyj0300
  
  // Need to fetch transactions without composite filter since it requires index.
  // Oh wait, my query for transactions had no index? But it said 0 txs.
  // Let me just fetch wallets and bonuses again.
  let query = {
    from: [{ collectionId: "bonuses" }],
    where: {
      compositeFilter: {
        op: "AND",
        filters: [{ fieldFilter: { field: { fieldPath: "userId" }, op: "EQUAL", value: { stringValue: uid } } }]
      }
    },
    limit: 5000
  };
  const res = await fetch(`https://firestore.googleapis.com/v1/projects/${SERVICE_ACCOUNT.project_id}/databases/(default)/documents:runQuery`, {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ structuredQuery: query })
  });
  const data = await res.json();
  const bonuses = data.filter(d => d.document).map(d => {
    const obj = {};
    for (const k in d.document.fields) obj[k] = fromFirestoreValue(d.document.fields[k]);
    return obj;
  });
  
  let rankTotal = 0;
  let missingTotal = 0;
  bonuses.forEach(b => {
     if (b.type === 'rank_bonus') {
         rankTotal += (b.amountUsdt || b.amount || 0);
     }
  });
  
  // Wait, I saw bonusBalance is $646.13, totalEarnings $360.96. 
  // What if bonusBalance was manually modified? Or is the difference $285 coming from somewhere else?
  console.log(`Rank Bonus USDT: $${rankTotal}`);
  
  // Let's get "users" document to see if they have any manual settings.
  const userRes = await fetch(`https://firestore.googleapis.com/v1/projects/${SERVICE_ACCOUNT.project_id}/databases/(default)/documents/users/${uid}`, {
      headers: { 'Authorization': `Bearer ${token}` }
  });
  const u = await userRes.json();
  const userData = {};
  if(u.fields) {
      for(const k in u.fields) userData[k] = fromFirestoreValue(u.fields[k]);
  }
  console.log("User Data:", userData.rank, userData.status);
}
run();
