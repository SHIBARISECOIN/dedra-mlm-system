...
async function run() {
  ...
  data.forEach(item => {
    if (item.document) {
      ...
      let totalDeposit = 0;
      if (fields.totalDeposit) {
        if (fields.totalDeposit.integerValue) totalDeposit = parseInt(fields.totalDeposit.integerValue, 10);
        else if (fields.totalDeposit.doubleValue) totalDeposit = parseFloat(fields.totalDeposit.doubleValue);
      }
      const uid = doc.name.split('/').pop();
      const id = fields.id?.stringValue || '';
      
      if (totalDeposit >= 20000) {
         console.log(`User: uid=${uid} id=${id}, deposit=${totalDeposit}, join=${fields.createdAt?.timestampValue}`);
      }
    }
  });
}
