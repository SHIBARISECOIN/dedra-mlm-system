...
async function run() {
  ...
  let bRes = await fetch(qUrl, {
    ...
    body: JSON.stringify({
      structuredQuery: {
        ...
        where: { fieldFilter: { field: { fieldPath: 'userId' }, op: 'EQUAL', value: { stringValue: 'pgUN1pS3cSUwzPYc5DjHKMZWIbE2' } } } // btc001
      }
    })
  });
  ...
  bonuses.filter(b => b.type === 'rank_bonus' || b.type === 'rank_matching').sort((a,b) => a.date.localeCompare(b.date)).forEach(b => {
