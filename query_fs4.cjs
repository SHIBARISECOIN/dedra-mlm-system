...
async function run() {
  ...
  data.forEach(item => {
    if (item.document) {
      ...
      // We look for users created early March
      if (createdAt.startsWith('2026-03-04') || createdAt.startsWith('2026-03-05') || createdAt.startsWith('2026-03-06')) {
         console.log(`User early: uid=${uid} id=${id}, join=${createdAt}`);
      }
    }
  });
}
