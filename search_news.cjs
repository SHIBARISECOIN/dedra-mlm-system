...
function fsQueryAllNews(token, projectId) {
  return new Promise((resolve, reject) => {
...
(async () => {
  try {
    ...
    const docs = await fsQueryAllNews(token, projectId);
    
    docs.forEach(d => {
      if (d.document) {
        const id = d.document.name.split('/').pop();
        const f = d.document.fields;
        const titleKo = f.title?.stringValue || '';
        if (titleKo.includes('난이도') || titleKo.includes('야당') || titleKo.includes('공포·탐욕')) {
            console.log(`ID: ${id}`);
            console.log(`Title: ${titleKo}`);
            console.log(`Title(EN): ${f.title_en?.stringValue}`);
        }
      }
    });
  } catch(e) {
