...
async function run() {
  ...
  // Maybe there are manual_adjusts not properly filtered? Let's get ALL transactions for user. No index? Just fetch 100 recent.
  let query = {
  ...
  const txs = data.filter(d => d.document && fromFirestoreValue(d.document.fields.userId) === uid).map(d => {
    const obj = {};
    for (const k in d.document.fields) obj[k] = fromFirestoreValue(d.document.fields[k]);
    return obj;
  });
  
  console.log(`Transactions found for cyj0300: ${txs.length}`);
