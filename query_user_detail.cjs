...
async function run() {
  ...
  data.filter(d => d.document).forEach(d => {
    ...
    if(type === 'roi' && new Date(createdAtStr) >= startOfToday) {
       if(f.userId?.stringValue === 'P7PfCWTtt6ZtllL6xGlSycVfqK93') {
           console.log(`Amount: ${f.amount?.doubleValue || f.amount?.integerValue}, Date: ${createdAtStr}, Reason: ${f.reason?.stringValue}`);
       }
    }
  });
}
