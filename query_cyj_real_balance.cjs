...
async function fsQueryFast(collection, token, uid) {
  let query = {
...
async function fsPatch(collection, docId, fields, token) {
  ...
  for (const [k, v] of Object.entries(fields)) {
    if (typeof v === 'number') firestoreFields[k] = { doubleValue: v };
...
async function run() {
  ...
  // 1. Get all true bonuses for this user that still exist
  const bonuses = await fsQueryFast('bonuses', token, uid);
  
  let totalTrueBonus = 0;
  for (const b of bonuses) {
    totalTrueBonus += Number(b.amount || b.amountUsdt || 0);
  }
  
  console.log(`True recalculation for cyj0300...`);
  console.log(`Remaining valid bonuses count: ${bonuses.length}`);
  console.log(`Real Bonus Total: $${totalTrueBonus.toFixed(2)}`);
  
  // 2. Restore his correct balance
  // Since we subtracted EVERYTHING, it might have wiped out his valid $111 from ROI and direct bonuses!
  // If the result of the minus was less than 0, Math.max(0, val) set it to 0.
  await fsPatch('wallets', uid, { 
    bonusBalance: totalTrueBonus,
    totalEarnings: totalTrueBonus
  }, token);
  
  console.log(`Restored cyj0300 wallet bonusBalance to $${totalTrueBonus.toFixed(2)}`);
}
run();
