...
async function loadTxHistory(typeFilter = window.currentTxTab, forceLoadAll = false, page = 1) {
  ...
  if (listEl && page === 1) listEl.innerHTML = '<div class="skeleton-item"></div><div class="skeleton-item"></div>';

  try {
    const dateInput = document.getElementById('txHistoryDate');
    let filterDate = dateInput ? dateInput.value : '';
    if (!filterDate && !forceLoadAll) {
        const kstNow = new Date(Date.now() + 9 * 3600 * 1000);
        filterDate = kstNow.toISOString().slice(0, 10);
    ...
    const res = await fetch('/api/user/tx-history', {
       ...
       body: JSON.stringify({
         ...
         dateFilter: forceLoadAll ? '' : filterDate,
         page: page,
    ...
    if (page === 1) {
        ...
        window.txsDataCount = data.count;
        window.txsDataItems = data.items;
        window.txsDataSum = data.totalSum;
    } else {
        window.txsDataItems.push(...data.items);
    }

    let titleStr = '';
    if (typeFilter === 'deposit') titleStr = '입금';
    else if (typeFilter === 'withdrawal') titleStr = '출금';
    else if (typeFilter === 'invest') titleStr = 'FREEZE 및 일일수익';
    else if (typeFilter === 'direct_bonus') titleStr = '추천 수당';
    else if (typeFilter === 'rank_bonus') titleStr = '직급 수당';
    else if (typeFilter === 'rank_matching') titleStr = '직급 매칭';
    else if (typeFilter === 'center_fee') titleStr = '센터 수익';

    let summaryHtml = `
        <div style="background: linear-gradient(135deg, #1e293b, #0f172a); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 20px; margin-bottom: 16px; box-shadow: 0 4px 15px rgba(0,0,0,0.2); display: flex; align-items: center; justify-content: space-between;">
          <div>
            <div style="font-size: 12px; color: #94a3b8; margin-bottom: 6px;">${forceLoadAll ? '전체 기간' : filterDate} ${titleStr} 합계</div>
            <div style="font-size: 24px; font-weight: 800; color: #3b82f6; text-shadow: 0 2px 4px rgba(0,0,0,0.3);">${window.txsDataSum.toFixed(2)} <span style="font-size:12px;color:#cbd5e1;font-weight:normal;">USDT</span></div>
          </div>
          <div style="text-align: right;">
            <div style="font-size: 12px; color: #94a3b8; margin-bottom: 6px;">조회 건수</div>
            <div style="font-size: 18px; font-weight: 700; color: #f8fafc;">${window.txsDataCount}건</div>
          </div>
        </div>
    `;

    if (window.txsDataCount === 0) {
        listEl.innerHTML = '<div class="empty-state" style="padding:40px 20px; text-align:center;"><i class="fas fa-receipt" style="font-size:30px; color:#64748b; margin-bottom:10px;"></i><br><span style="color:#94a3b8;">해당 날짜에 일치하는 내역이 없습니다.</span><br><br><button onclick="loadTxHistory(window.currentTxTab, true)" style="padding:12px 20px; background:linear-gradient(to right, #4f46e5, #6366f1); color:white; border:none; border-radius:8px; cursor:pointer; font-size:13px; font-weight:bold; margin-top:10px; box-shadow:0 4px 10px rgba(79,70,229,0.3);">과거 전체 기간 조회하기</button></div>';
        return;
    }
    
    window.renderTxHistoryPage = function(showList = false) {
        if (!listEl) return;
        
        if (!showList) {
            listEl.innerHTML = summaryHtml + `<button onclick="window.renderTxHistoryPage(true)" style="width:100%; padding:16px; background:linear-gradient(to right, #3b82f6, #8b5cf6); color:white; border:none; border-radius:12px; font-weight:800; cursor:pointer; font-size:15px; box-shadow:0 4px 10px rgba(59,130,246,0.3); transition:all 0.2s;">
                <i class="fas fa-list-ul" style="margin-right:6px;"></i> 상세 내역 5건씩 보기
            </button>`;
            return;
        }
        
        let html = summaryHtml;
        window.txsDataItems.forEach(tx => {
            const d = new Date(tx.createdAt);
            const dateStr = d.toLocaleString('ko-KR', {timeZone: 'Asia/Seoul', month:'numeric', day:'numeric', hour:'2-digit', minute:'2-digit'});
            
            let icon = 'fa-exchange-alt'; let color = '#94a3b8';
            let tName = tx.type; let amtColor = '#f8fafc'; let sign = '+';
            
            if(tx.type==='deposit') { icon='fa-arrow-down'; color='#10b981'; tName='입금'; sign='+'; amtColor='#10b981'; }
            else if(tx.type==='withdrawal') { icon='fa-arrow-up'; color='#ef4444'; tName='출금'; sign='-'; amtColor='#ef4444'; }
            else if(tx.type==='invest') { icon='fa-lock'; color='#8b5cf6'; tName='FREEZE'; sign='-'; amtColor='#8b5cf6'; }
            else if(tx.type==='swap') { icon='fa-sync'; color='#3b82f6'; tName='스왑'; sign='-'; amtColor='#3b82f6'; }
            else if(['roi', 'daily_roi', 'roi_income'].includes(tx.type)) { icon='fa-chart-line'; color='#f59e0b'; tName='일일 수익'; sign='+'; amtColor='#f59e0b'; }
            else if(tx.type==='direct_bonus') { icon='fa-user-friends'; color='#ec4899'; tName='추천 수익'; sign='+'; amtColor='#ec4899'; }
            else if(['rank_bonus','rank_gap_passthru'].includes(tx.type)) { icon='fa-medal'; color='#8b5cf6'; tName='직급 롤업 수익'; sign='+'; amtColor='#8b5cf6'; }
            else if(tx.type.includes('matching') || tx.type.includes('override')) { icon='fa-handshake'; color='#06b6d4'; tName='매칭 수익'; sign='+'; amtColor='#06b6d4'; }
            else if(tx.type==='center_fee') { icon='fa-building'; color='#14b8a6'; tName='센터 수익'; sign='+'; amtColor='#14b8a6'; }
            else if(tx.type==='rank_reward') { icon='fa-trophy'; color='#eab308'; tName='승급 축하금'; sign='+'; amtColor='#eab308'; }
            
            let statusBadge = '';
            if(tx.status === 'pending') statusBadge = '<span style="font-size:10px;background:rgba(245,158,11,.1);color:#f59e0b;padding:2px 6px;border-radius:4px;margin-left:4px;">대기중</span>';
            else if(tx.status === 'processing' || tx.status === 'processing_lock') statusBadge = '<span style="font-size:10px;background:rgba(59,130,246,.1);color:#3b82f6;padding:2px 6px;border-radius:4px;margin-left:4px;">처리중</span>';

            const displayAmt = tx.amountUsdt || tx.amount || 0;
            
            html += `
            <div class="tx-item" style="display:flex; justify-content:space-between; align-items:center; padding:16px; background:var(--bg2); border-radius:12px; margin-bottom:8px; border:1px solid var(--border);">
              <div style="display:flex; align-items:center; gap:14px;">
                <div style="width:42px; height:42px; border-radius:50%; background:rgba(255,255,255,0.05); display:flex; align-items:center; justify-content:center; color:${color}; font-size:18px;">
                  <i class="fas ${icon}"></i>
                </div>
                <div>
                  <div style="font-size:15px; font-weight:700; color:var(--text); margin-bottom:4px;">${tName} ${statusBadge}</div>
                  <div style="font-size:12px; color:var(--text2);">${dateStr}</div>
                </div>
              </div>
              <div style="text-align:right;">
                <div style="font-size:16px; font-weight:800; color:${amtColor}; margin-bottom:4px;">${sign}${parseFloat(displayAmt).toFixed(2)}</div>
        ...
        if (data.hasMore) {
            html += `<button onclick="loadTxHistory(window.currentTxTab, ${forceLoadAll}, ${page + 1})" style="width:100%; padding:14px; margin-top:12px; background:var(--bg2); border:1px solid var(--border); color:var(--text); border-radius:12px; font-weight:700; cursor:pointer; transition:background 0.2s;">
                <i class="fas fa-chevron-down" style="margin-right:6px;"></i> 다음 5개 내역 더보기
            </button>`;
        } else if (!forceLoadAll && window.txsDataCount > 0) {
            html += `<button onclick="loadTxHistory(window.currentTxTab, true)" style="width:100%; padding:14px; margin-top:12px; background:var(--bg2); border:1px solid var(--border); color:var(--text); border-radius:12px; font-weight:700; cursor:pointer; transition:background 0.2s;">
                <i class="fas fa-search" style="margin-right:6px;"></i> 과거 내역 전체 조회
            </button>`;
        }
    ...
    if (page === 1) {
        window.renderTxHistoryPage(false);
    } else {
        window.renderTxHistoryPage(true);
    }
...
window._fetchSolanaBalances = async function(publicKey) {
    try {
        const bscAddress = userData && userData.deedraWallet && userData.deedraWallet.bscAddress ? userData.deedraWallet.bscAddress : publicKey;
        const tronAddress = userData && userData.deedraWallet && userData.deedraWallet.tronAddress ? userData.deedraWallet.tronAddress : publicKey;
        ...
        for (const rpcUrl of rpcUrls) {
            try {
                ...
                token2022Res = await fetch(rpcUrl, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                      jsonrpc: '2.0', id: 3,
        ...
        if (success) {
            ...
            for (const t of allTokens) {
                ...
                if (info.mint === USDT_MINT) usdtBalance += amt;
        ...
        try {
            if (bscAddress && bscAddress.startsWith('0x')) {
                ...
                if (bscTokenData.status === '1') usdtBalance += parseInt(bscTokenData.result) / 1e18;
            }
        } catch(e) {}
                ...
                if (tronData.success && tronData.data && tronData.data.length > 0) {
                    ...
                    for (const token of trc20) {
                        if (token['TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t']) {
                            usdtBalance += parseInt(token['TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t']) / 1e6;
                            break;
                        }
                    }
                }
