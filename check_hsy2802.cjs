...
async function checkUser() {
  try {
    const email = 'hsy2802@deedra.com';
    ...
    let totalBet = 0;
    let totalWon = 0; // The amount they received when they won
    let netProfitDdra = 0; // User's net profit (DDRA)
    let gamesCount = {
      baccarat: 0,
      roulette: 0,
      slot: 0,
    ...
    logsSnapshot.forEach(doc => {
      const log = doc.data();
      const bet = log.bet || 0;
      const change = log.ddraChange || 0; // Positive for win net, negative for loss
    ...
    const txSnapshot = await txRef.where('type', '==', 'withdrawal').orderBy('createdAt', 'desc').limit(5).get();
    ...
    if (txSnapshot.empty) {
      ...
      txSnapshot.forEach(doc => {
        const tx = doc.data();
        console.log(`- Amount: ${tx.amount} | AmountUSDT: ${tx.amountUsdt} | Status: ${tx.status} | Date: ${tx.createdAt ? tx.createdAt.toDate().toISOString() : 'N/A'}`);
      });
    }
