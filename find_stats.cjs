...
async function fsQueryAll(collection, token) {
  let query = { from: [{ collectionId: collection }], limit: 100 };
...
async function run() {
  ...
  for (const u of users) {
    if (u.downlineInvestStats || u.legStats || u.totalInvest) {
      console.log(u.email, 'has stats:', Object.keys(u).filter(k => k.toLowerCase().includes('stat') || k.toLowerCase().includes('leg')));
    }
  }
}
run();
