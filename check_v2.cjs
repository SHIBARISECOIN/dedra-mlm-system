...
async function fsGet(collection, docId, token) {
  const res = await fetch(`https://firestore.googleapis.com/v1/projects/dedra-mlm/databases/(default)/documents/${collection}/${docId}`, {
  ...
  const obj = { id: docId };
  if(data.fields) {
    for (const k in data.fields) obj[k] = fromFirestoreValue(data.fields[k]);
  }
  return obj;
}
...
async function run() {
  ...
  const user = await fsGet('users', 'v2rX4gUquYeAjcr1paSIOKKgNU92', token);
  console.log('v2rx user:', user.name, user.referralUserId, user.parentId, user.sponsorId, user.referralCode);
  
  const cyj = await fsGet('users', 'qAdGKU772oVGZ0B5PwUEbL3UqSF3', token);
  console.log('cyj0300 code:', cyj.referralCode);
}
run();
