...
async function run() {
  try {
    ...
    console.log("Token acquired, resuming deletion...");
    ...
    console.log(`Found ${badBonuses.length} remaining bad matching bonuses to rollback.`);
    
    // Wallets are already updated from previous run, we only need to delete the docs.
    
    console.log("\nStarting Bonus Deletion...");
    let delCount = 0;
    // execute in parallel batches to be faster
    const BATCH_SIZE = 50;
    for (let i = 0; i < badBonuses.length; i += BATCH_SIZE) {
      const batch = badBonuses.slice(i, i + BATCH_SIZE);
      await Promise.all(batch.map(b => fsDelete('bonuses', b.id, token)));
      delCount += batch.length;
      console.log(`Deleted ${delCount}/${badBonuses.length}`);
    }
    
    console.log(`\nROLLBACK COMPLETE! Deleted remaining ${delCount} bonus records.`);
