...
async function run() {
  const annSnap = await db.collection('announcements').where('isActive', '==', true).get();
  console.log('Query Announcements:', annSnap.size);
  const prodSnap = await db.collection('products').where('isActive', '==', true).get();
  console.log('Query Products:', prodSnap.size);
}
run();
