...
// [same getAdminToken and fromFirestoreValue]
...
async function run() {
  ...
  const users = await fsQueryAll('users', token);
  ...
  for (const u of users) {
    if (u.referralUserId) {
      if (!childrenMap[u.referralUserId]) childrenMap[u.referralUserId] = [];
      childrenMap[u.referralUserId].push(u.id);
    }
  }
  
  // Calculate total volume recursively
  function getSubVolume(uid) {
    let vol = Number(walletMap[uid]?.totalInvest || 0);
    const children = childrenMap[uid] || [];
    for (const childId of children) {
      vol += getSubVolume(childId);
    }
    return vol;
  }
  
  const targetId = 'qAdGKU772oVGZ0B5PwUEbL3UqSF3'; // cyj0300
  const cyj = userMap[targetId];
  const myInvest = Number(walletMap[targetId]?.totalInvest || 0);
  console.log(`User: ${cyj.name} (${cyj.email})`);
  console.log(`My Invest: $${myInvest}`);
  
  const children = childrenMap[targetId] || [];
  ...
  for (const childId of children) {
    ...
    const vol = getSubVolume(childId);
    legs.push({ name: child.name, email: child.email, volume: vol });
  }
  
  legs.sort((a,b) => b.volume - a.volume);
  legs.forEach((l, idx) => {
    console.log(`Leg ${idx+1} (${l.name}): $${l.volume}`);
  });
}
run();
