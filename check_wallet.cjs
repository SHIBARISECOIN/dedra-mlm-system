...
async function run() {
  ...
  const res = await fetch(`https://firestore.googleapis.com/v1/projects/${SERVICE_ACCOUNT.project_id}/databases/(default)/documents/wallets/qAdGKU772oVGZ0B5PwUEbL3UqSF3`, {
  ...
  if (doc.fields) {
      ...
      for(const k in doc.fields) {
        parsed[k] = convertValue(doc.fields[k]);
      }
      console.log("Wallet for CYJ0300:");
      console.log(parsed);
  } else {
      console.log("No wallet found or error:", doc);
  }
}
