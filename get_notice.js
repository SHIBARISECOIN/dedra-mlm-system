...
async function run() {
  ...
  snap.forEach(d => {
    const data = d.data();
    if (data.title && (data.title.includes('하락장') || data.title.includes('Bear Market'))) {
      console.log('ID:', d.id);
      console.log('Title (KR):', data.title);
      console.log('Content (KR):', data.content.substring(0, 150) + '...');
    }
  });
}
run();
