...
async function run() {
  try {
    ...
    // fetch users to map username -> uid
    ...
    console.log(`Fixing notifications for ${uidsToNotify.length} users...`);
    ...
    for (const uid of uidsToNotify) {
        ...
        let bodyLines = ['이전 사이트의 보유 상품이 성공적으로 이전되었습니다. 📦<br>'];
        let totalCount = 0;
        let totalAmount = 0;
        ...
        bodyLines.push(`<br>총 ${totalCount}개의 상품(${totalAmount} USDT)이 정상 연동되어, 하단 [FREEZE] 탭에서 확인 및 일일 수익이 정상 지급됩니다.`);
        
        // Use <br> instead of \n for HTML rendering in app.js
