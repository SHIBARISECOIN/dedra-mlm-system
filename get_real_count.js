import admin from 'firebase-admin';

const serviceAccount = {
  type: "service_account",
  project_id: "dedra-mlm",
  private_key_id: "9bf051a4b3416458b8e7b18da5f51565fd5bad13",
  private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  client_email: "firebase-adminsdk-fbsvc@dedra-mlm.iam.gserviceaccount.com"
};

admin.initializeApp({
  credential: admin.credential.cert(serviceAccount)
});

const db = admin.firestore();

async function run() {
  const jpRef = db.collection('events').doc('jackpot');
  
  // Get start of today (midnight)
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  
  const invSnap = await db.collection('investments')
    .where('createdAt', '>=', admin.firestore.Timestamp.fromDate(today))
    .orderBy('createdAt', 'asc')
    .get();
    
  let changeCount = 0;
  let lastUser = null;
  let count = 0;
  
  invSnap.forEach(doc => {
    const data = doc.data();
    count++;
    if (lastUser !== data.userId) {
      changeCount++;
      lastUser = data.userId;
    }
  });
  
  console.log(`Total investments today: ${count}`);
  console.log(`Actual change count today: ${changeCount}`);
  
  // If we want to set it to exact real count
  // First person doesn't count as a "change" from someone else, so max(0, changeCount - 1)
  const realChanges = Math.max(0, changeCount - 1);
  console.log(`Real changes (excluding first setter): ${realChanges}`);
  
  await jpRef.update({
    changeCount: realChanges
  });
  
  console.log(`Updated to ${realChanges}`);
  process.exit(0);
}

run().catch(console.error);
