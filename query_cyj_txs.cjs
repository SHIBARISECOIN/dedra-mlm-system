...
async function run() {
  ...
  // Actually, we already saw Bonus Balance is $646.
  // And we saw "Total Bonuses Calculated: $360.97"
  // Where does the other $285 come from? Maybe old bonuses? 
  // Let's fetch all bonuses WITHOUT limit. 
  ...
  console.log(`Total Bonuses Count: ${bonuses.length}`);
  console.log(`Total Bonuses USDT: $${sumUsdt}`);
  console.log(summary);
}
run();
