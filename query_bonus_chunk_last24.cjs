...
async function run() {
        ...
        where: {
          compositeFilter: {
            ...
            filters: [
              ...
              { fieldFilter: { field: { fieldPath: 'createdAt' }, op: 'GREATER_THAN_OR_EQUAL', value: { stringValue: oneDayAgo } } }
            ]
          }
        },
        limit: 10000
  ...
  let bonuses = data.filter(d => d.document).map(d => fsDocToObj(d.document));
  console.log(`Found ${bonuses.length} ROI bonuses since yesterday.`);
  ...
  for (const b of bonuses) {
    if (!userMap[b.userId]) userMap[b.userId] = { totalRoiUsdt: 0, count: 0, days: [] };
  ...
  console.log("-------------------");
  fs.writeFileSync('/home/user/webapp/today_bonus_report_v2.json', JSON.stringify(results, null, 2));
}
