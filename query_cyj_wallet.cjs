...
async function run() {
  ...
  let txTotal = 0;
  txs.forEach(t => {
      console.log(`TX [${t.type}]: +${t.amountUsdt || t.amount} (${t.status}) - ${t.reason || t.memo}`);
      if(t.status === 'approved' && ['manual_add', 'bonus', 'roi'].includes(t.type)) {
          txTotal += (t.amountUsdt || t.amount || 0);
      }
  });
  console.log(`Total from Txs: $${txTotal}`);
}
run();
