import admin from 'firebase-admin';

const serviceAccount = {
  type: "service_account",
  project_id: "dedra-mlm",
  private_key_id: "9bf051a4b3416458b8e7b18da5f51565fd5bad13",
  private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  client_email: "firebase-adminsdk-fbsvc@dedra-mlm.iam.gserviceaccount.com"
};

admin.initializeApp({
  credential: admin.credential.cert(serviceAccount)
});

const db = admin.firestore();

async function run() {
  const prodSnap = await db.collection('products').get();
  console.log('Total Products:', prodSnap.size);
  prodSnap.forEach(d => console.log('Product:', d.id, 'isActive=', d.data().isActive, 'type=', typeof d.data().isActive));

  const annSnap = await db.collection('announcements').get();
  console.log('Total Announcements:', annSnap.size);
  annSnap.forEach(d => console.log('Announcement:', d.id, 'isActive=', d.data().isActive, 'type=', typeof d.data().isActive));
}
run();
