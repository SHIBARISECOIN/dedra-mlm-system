// check manual_adjust in wallets directly maybe?
// Wait, bonusBalance is 646.13. Total Bonuses is 360.96. The difference is 285.17.
...
async function run() {
  ...
  let bal = 0;
  txs.forEach(t => {
      console.log(`TX [${t.type}] Amount: ${t.amountUsdt || t.amount} (${t.status}) - ${t.reason || t.memo || ''}`);
  });
}
run();
