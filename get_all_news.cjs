const https = require('https');

const SERVICE_ACCOUNT = {
  type: "service_account",
  project_id: "dedra-mlm",
  private_key_id: "9bf051a4b3416458b8e7b18da5f51565fd5bad13",
  private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  client_email: "firebase-adminsdk-fbsvc@dedra-mlm.iam.gserviceaccount.com",
  client_id: "103096684164693920388",
  token_uri: "https://oauth2.googleapis.com/token",
  project_id_full: "dedra-mlm"
};

function base64urlurlEncode(str) {
  return Buffer.from(str).toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
}

function createJwt(serviceAccount) {
  const header = { alg: 'RS256', typ: 'JWT' };
  const now = Math.floor(Date.now() / 1000);
  const claim = {
    iss: serviceAccount.client_email,
    scope: 'https://www.googleapis.com/auth/datastore',
    aud: 'https://oauth2.googleapis.com/token',
    exp: now + 3600,
    iat: now
  };
  return base64urlurlEncode(JSON.stringify(header)) + '.' + base64urlurlEncode(JSON.stringify(claim)) + '.' + require('crypto').createSign('RSA-SHA256').update(base64urlurlEncode(JSON.stringify(header)) + '.' + base64urlurlEncode(JSON.stringify(claim))).sign(serviceAccount.private_key, 'base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
}

function getAccessToken(jwt) {
  return new Promise((resolve, reject) => {
    const req = https.request({
      hostname: 'oauth2.googleapis.com', path: '/token', method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
    }, res => { let b = ''; res.on('data', d => b+=d); res.on('end', () => resolve(JSON.parse(b).access_token)); });
    req.write('grant_type=urn%3Aietf%3Aparams%3Aoauth%3Agrant-type%3Ajwt-bearer&assertion=' + jwt);
    req.end();
  });
}

(async () => {
  const token = await getAccessToken(createJwt(SERVICE_ACCOUNT));
  const req = https.request({
    hostname: 'firestore.googleapis.com',
    path: `/v1/projects/${SERVICE_ACCOUNT.project_id}/databases/(default)/documents:runQuery`,
    method: 'POST',
    headers: { 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' }
  }, res => {
    let b = ''; res.on('data', d => b+=d);
    res.on('end', () => {
      const docs = JSON.parse(b);
      docs.forEach(d => {
        if (d.document) {
          const f = d.document.fields;
          console.log(d.document.name.split('/').pop(), " | KO: ", f.title?.stringValue, " | EN: ", f.title_en?.stringValue);
        }
      });
    });
  });
  req.write(JSON.stringify({ structuredQuery: { from: [{ collectionId: 'news' }] } }));
  req.end();
})();
