...
async function banAndConfiscate() {
  try {
    ...
    const notifMsg = `[안내] 시스템 오류 악용(어뷰징)에 따른 계정 영구 정지 및 부당 이득 환수 통보\n\n` +
      `당사 보안 및 모니터링 시스템 감식 결과, 해당 계정에서 정상적인 서비스 이용이 아닌 시스템의 특정 확률 오류(맹점)를 고의적으로 악용한 심각한 어뷰징(Abusing) 행위가 적발되었습니다.\n\n` +
      `- 비정상적인 게임 패턴: 시스템 버그를 악용하여 확률이 잘못 적용된 게임에만 집중적으로 배팅 (총 3,786회 플레이)\n` +
      `- 약관 위반: 시스템의 취약점 및 버그를 고의적으로 악용하여 부당한 이득을 취하는 행위\n\n` +
