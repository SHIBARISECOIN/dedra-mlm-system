...
async function run() {
  ...
  let uRes = await fetch(qUrl, {
    ...
    body: JSON.stringify({
      structuredQuery: {
        ...
        where: { fieldFilter: { field: { fieldPath: 'id' }, op: 'EQUAL', value: { stringValue: 'btc001' } } },
        limit: 1
      }
    })
  });
  ...
  if (uData[0] && uData[0].document) {
      ...
      console.log("Found btc001 uid:", uid);
  } else {
      console.log("User btc001 not found by id field, trying email prefix...");
      // Let's dump all users and find it.
  }
  
  if (!uid) {
      const allUsersFile = fs.readFileSync('/home/user/webapp/users_list.json', 'utf8');
      const lines = allUsersFile.split('\n');
      for (const line of lines) {
          if (line.includes('btc001')) {
              console.log(line);
