...
async function run() {
  ...
  res = await fetch(`${FIRESTORE_BASE}:runQuery`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ structuredQuery: { from: [{ collectionId: 'investments' }], limit: 10000 } })
  });
  data = await res.json();
  let invs = data.filter(d => d.document).map(d => fsDocToObj(d.document));
  
  const todayStr = new Date().toISOString().slice(0, 10);
  const todayBonuses = bonuses.filter(b => b.type === 'roi' && b.createdAt && String(b.createdAt).startsWith(todayStr));
  ...
  console.log(`총 ${results.length}명 정산 내역 분석 (오늘: ${todayStr})`);
