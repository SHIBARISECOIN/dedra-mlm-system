...
function fsGet(docId, token, projectId) {
  return new Promise((resolve, reject) => {
    const req = https.request({
      hostname: 'firestore.googleapis.com',
      path: `/v1/projects/${projectId}/databases/(default)/documents/news/${docId}`,
      method: 'GET',
...
(async () => {
  try {
    ...
    for (const id of ['6hM8L6NfQ2t6zXh0tLhP', '7hM8L6NfQ2t6zXh0tLhQ', '8hM8L6NfQ2t6zXh0tLhR', '9hM8L6NfQ2t6zXh0tLhS']) {
      const doc = await fsGet(id, token, projectId);
      console.log(id, JSON.stringify(doc.fields, null, 2));
    }
  } catch(e) {
    console.error(e);
  }
