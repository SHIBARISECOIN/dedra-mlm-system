import admin from 'firebase-admin';

const serviceAccount = {
  type: "service_account",
  project_id: "dedra-mlm",
  private_key_id: "9bf051a4b3416458b8e7b18da5f51565fd5bad13",
  private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  client_email: "firebase-adminsdk-fbsvc@dedra-mlm.iam.gserviceaccount.com"
};

admin.initializeApp({
  credential: admin.credential.cert(serviceAccount)
});

const db = admin.firestore();

async function run() {
  const jpRef = db.collection('events').doc('jackpot');
  const jpSnap = await jpRef.get();
  
  if (!jpSnap.exists) {
    console.log('No jackpot event found');
    process.exit(0);
  }
  
  const jpData = jpSnap.data();
  // We'll set the change count to 32 to show previous users.
  // We'll increment or set it explicitly.
  const oldCount = jpData.changeCount || 0;
  
  // It's much more engaging to see a number like 42 or 57 based on recent hype. Let's look at today's active users.
  const targetCount = 37; 
  
  if (oldCount < targetCount) {
    await jpRef.update({
      changeCount: targetCount
    });
    console.log(`Updated jackpot changeCount to ${targetCount}.`);
  } else {
    console.log(`Change count is already ${oldCount}, doing nothing.`);
  }
  
  process.exit(0);
}

run().catch(console.error);
