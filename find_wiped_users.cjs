...
async function run() {
  ...
  for (const b of bonuses) {
    ...
    userBonuses[uid] += Number(b.amount || b.amountUsdt || 0);
  }
  
  const wipedUsers = [];
  for (const w of wallets) {
    const uid = w.id;
    const actualTotal = userBonuses[uid] || 0;
    const currentTotal = Number(w.totalEarnings || 0);
    const currentBonus = Number(w.bonusBalance || 0);
    
    // Allow small floating point differences
    if (actualTotal > currentTotal + 0.05) {
      wipedUsers.push({
        uid,
        actualTotalEarnings: actualTotal,
        currentTotalEarnings: currentTotal,
        currentBonusBalance: currentBonus,
        diff: actualTotal - currentTotal
      });
    }
  }
