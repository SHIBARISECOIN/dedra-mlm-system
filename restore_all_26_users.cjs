...
async function fsPatch(collection, docId, fields, token) {
  ...
  await fetch(`https://firestore.googleapis.com/v1/projects/dedra-mlm/databases/(default)/documents/${collection}/${docId}?${updateMask}`, {
    method: 'PATCH',
    headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
...
async function run() {
  ...
  for (const w of wallets) {
    const uid = w.id;
    // skip cyj0300 because we already fixed him manually and we don't want to mess it up just in case
    if (uid === 'qAdGKU772oVGZ0B5PwUEbL3UqSF3') continue;

    const actualTotalEarnings = userBonuses[uid] || 0;
    const currentTotal = Number(w.totalEarnings || 0);
    
    // Any user whose totalEarnings was incorrectly dropped below their valid bonuses
    if (actualTotalEarnings > currentTotal + 0.1) {
      const calculatedBonus = actualTotalEarnings + (userTxDeltas[uid] || 0);
      const newBonusBalance = Math.max(0, calculatedBonus);
      
      console.log(`Fixing User ${uid}`);
      console.log(`  Earnings: ${currentTotal} -> ${actualTotalEarnings}`);
      console.log(`  Bonus: ${Number(w.bonusBalance || 0)} -> ${newBonusBalance}`);
  ...
  console.log(`Successfully restored ${fixedCount} users!`);
}
run();
