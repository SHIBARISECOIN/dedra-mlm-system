...
async function run() {
  ...
  // Find who has cyj0300 as referralUserId or parentId
  const directRefs = users.filter(u => u.referralUserId === targetId);
  const directParents = users.filter(u => u.parentId === targetId);
  
  console.log(`Users with referralUserId = cyj0300: ${directRefs.length}`);
  console.log(`Users with parentId = cyj0300: ${directParents.length}`);
  
  if (directParents.length > 0) {
    console.log("Direct Parents mapping:");
    directParents.forEach(u => console.log(`- ${u.name} (${u.email})`));
  }
}
run();
