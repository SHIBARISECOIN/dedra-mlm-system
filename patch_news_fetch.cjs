...
const newLoadNews = `window.loadNewsFeed = async function(isRefresh = false) {
  ...
  try {
    ...
    const items = snap.docs.map(d => {
      ...
      return {
        ...
        pubDate: data.createdAt ? data.createdAt.toDate().toISOString() : new Date().toISOString()
      };
    });
...
console.log('patched loadNewsFeed');
