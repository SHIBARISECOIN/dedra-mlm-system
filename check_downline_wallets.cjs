...
async function run() {
  ...
  const wRes = await fetch(`https://firestore.googleapis.com/v1/projects/${SERVICE_ACCOUNT.project_id}/databases/(default)/documents/wallets?pageSize=1000`, {
    headers: { 'Authorization': `Bearer ${token}` }
  });
  const wData = await wRes.json();
  const wMap = {};
  wData.documents.forEach(doc => {
      const id = doc.name.split('/').pop();
      let parsed = {};
      if(doc.fields) {
          for(const k in doc.fields) parsed[k] = convertValue(doc.fields[k]);
      }
      wMap[id] = parsed;
  });
  ...
  data.documents.forEach(doc => {
    ...
    if (parsed.referredBy === uid) {
      console.log(`User: ${parsed.name} (${parsed.email}) ID: ${parsed.id}`);
      console.log(`Wallet totalInvest: ${wMap[parsed.id]?.totalInvest}, totalInvested: ${wMap[parsed.id]?.totalInvested}`);
    }
  });
}
