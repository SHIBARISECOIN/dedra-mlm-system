...
async function run() {
  ...
  data.forEach(item => {
    ...
    if (parsed.fromUserId) sources.add(parsed.fromUserId);
    if (parsed.fromUserName) sources.add(parsed.fromUserName);
    
    if (parsed.type === 'direct_bonus') {
      if (parsed.fromUserId) directSources.add(parsed.fromUserId);
      if (parsed.fromUserName) directSources.add(parsed.fromUserName);
    }
  });

  console.log(`CYJ0300 received bonuses from these users/IDs:`);
  console.log(Array.from(sources).join(', '));
  
  console.log(`\nDirect bonuses received from:`);
  console.log(Array.from(directSources).join(', '));
}
