...
const newsEndpoint = `app.post('/api/translate/news', async (c) => {
  try {
...
// Insert the new endpoint right after the announcement endpoint
if (!code.includes('/api/translate/news')) {
  code = code.replace(/app\.post\('\/api\/translate\/announcement'[\s\S]*?\}\)\n\)/, match => match + '\n\n' + newsEndpoint);
}
