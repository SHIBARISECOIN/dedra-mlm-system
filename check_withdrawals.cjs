...
async function run() {
  ...
  const tx = await fsQueryAll('transactions', token);
  console.log('Transactions sample:', tx.slice(0,3));
  const wd = await fsQueryAll('withdrawals', token);
  console.log('Withdrawals sample:', wd.slice(0,3));
}
run();
