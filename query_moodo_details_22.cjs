...
async function run() {
  ...
  let bRes = await fetch(qUrl, {
    ...
    body: JSON.stringify({
      structuredQuery: {
        ...
        where: { fieldFilter: { field: { fieldPath: 'userId' }, op: 'EQUAL', value: { stringValue: 'KM7pKoYx4lM2gUt1wKPgT2rl57M2' } } }
      }
    })
  });
  ...
  bonuses.filter(b => b.date === '2026-03-22').sort((a,b) => a.reason.localeCompare(b.reason)).forEach(b => {
