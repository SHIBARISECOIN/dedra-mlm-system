const admin = require('firebase-admin');

const SERVICE_ACCOUNT = {
  project_id: "dedra-mlm",
  private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  client_email: "firebase-adminsdk-fbsvc@dedra-mlm.iam.gserviceaccount.com",
  token_uri: "https://oauth2.googleapis.com/token"
};
const jwt = require('jsonwebtoken');

async function getAdminToken() {
  const payload = {
    iss: SERVICE_ACCOUNT.client_email,
    sub: SERVICE_ACCOUNT.client_email,
    aud: SERVICE_ACCOUNT.token_uri,
    iat: Math.floor(Date.now() / 1000),
    exp: Math.floor(Date.now() / 1000) + 3600,
    scope: 'https://www.googleapis.com/auth/datastore'
  };
  const token = jwt.sign(payload, SERVICE_ACCOUNT.private_key, { algorithm: 'RS256' });
  const res = await fetch(SERVICE_ACCOUNT.token_uri, {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    body: `grant_type=urn:ietf:params:oauth:grant-type:jwt-bearer&assertion=${token}`
  });
  return (await res.json()).access_token;
}

function fromFirestoreValue(val) {
  if (!val) return null;
  if ('stringValue' in val) return val.stringValue;
  if ('doubleValue' in val) return val.doubleValue;
  if ('integerValue' in val) return parseInt(val.integerValue,10);
  if ('timestampValue' in val) return new Date(val.timestampValue);
  return val;
}

async function run() {
  const token = await getAdminToken();
  const uid = 'qAdGKU772oVGZ0B5PwUEbL3UqSF3'; // cyj0300
  
  // Okay, wait. Maybe it's not transactions. What if manual_adjust directly added to bonusBalance?
  // Let me check logs? No, logs might be too much.
  // We verified total bonus records = $360.97
  // But the wallet bonusBalance = $646.13. Difference is exactly 285.17.
  // Was there a manual deposit or a withdrawal?
  // No transactions found.
  // Wait, `transactions` collection might have a different name in DB? 
  // Let's list collections: NO we can't easily.
  // Wait, let's query ANY transactions without composite filter.
  let query2 = {
    from: [{ collectionId: "transactions" }],
    limit: 5
  };
  const res2 = await fetch(`https://firestore.googleapis.com/v1/projects/${SERVICE_ACCOUNT.project_id}/databases/(default)/documents:runQuery`, {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ structuredQuery: query2 })
  });
  const data2 = await res2.json();
  console.log(data2[0]);
}
run();
